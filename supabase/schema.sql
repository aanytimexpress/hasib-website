...
create table if not exists public.roles (
  ...
  name text unique not null check (name in ('super_admin', 'editor', 'moderator', 'user')),
  label text not null,
...
declare
  ...
  admin_allowlist text[] := array[
    'aanytime.xpress@gmail.com',
    'alshakib730@gmail.com'
  ];
begin
  if lower(coalesce(new.email, '')) = any(admin_allowlist) then
